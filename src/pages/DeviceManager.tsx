...
import { CheckCircleIcon, AlertCircleIcon, XCircleIcon, BluetoothIcon, MonitorIcon, HardDriveIcon, CpuIcon, WifiIcon, UsbIcon } from 'lucide-react';
...
export function DeviceManager({
  ...
  const getDeviceIcon = (type: string) => {
    switch (type) {
      case 'Processor':
        return <CpuIcon size={48} className="text-cloud-blue" />;
      case 'Graphics':
        return <MonitorIcon size={48} className="text-cloud-purple" />;
      case 'Memory':
        return <HardDriveIcon size={48} className="text-cloud-green" />;
      case 'Network':
        return <WifiIcon size={48} className="text-cloud-blue" />;
      case 'Bluetooth':
        return <BluetoothIcon size={48} className="text-cloud-blue" />;
      case 'Display':
        return <MonitorIcon size={48} className="text-cloud-purple" />;
      case 'Storage':
        return <HardDriveIcon size={48} className="text-cloud-green" />;
      case 'USB':
        return <UsbIcon size={48} className="text-cloud-blue" />;
      default:
        return <MonitorIcon size={48} className="text-cloud-gray-deeper" />;
    }
  };
  ...
  const handleEnableDevice = () => {
    if (selectedDevice) {
      setDevices(devices.map(d => d.id === selectedDevice.id ? { ...d, status: 'working' as DeviceStatus } : d));
      setSelectedDevice({ ...selectedDevice, status: 'working' });
    }
  };
  
  const handleDisableDevice = () => {
    if (selectedDevice) {
      setDevices(devices.map(d => d.id === selectedDevice.id ? { ...d, status: 'disabled' as DeviceStatus } : d));
      setSelectedDevice({ ...selectedDevice, status: 'disabled' });
    }
  };
            ...
            {devices.map(device => (
              ...
              >
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 rounded-cloud-lg bg-cloud-blue/20 flex items-center justify-center flex-shrink-0">
