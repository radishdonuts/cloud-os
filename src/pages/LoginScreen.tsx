...
import { PowerIcon, KeyboardIcon, EyeIcon, EyeOffIcon } from 'lucide-react';
  ...
  return <div className="w-full min-h-screen bg-gradient-to-br from-cloud-cream via-cloud-green/20 to-cloud-blue/20 flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md px-6 animate-fade-in">
        <div className="bg-white/80 backdrop-blur-cloud rounded-cloud-xl shadow-cloud-lg p-8 border border-cloud-gray/20">
          ...
          {/* User Avatar */}
          <div className="flex justify-center mb-6">
            <Avatar size="xl" fallback="JD" online />
          </div>
          ...
          <div className="mb-6 relative">
            <Input type={showPassword ? 'text' : 'password'} placeholder="Enter your password" value={password} onChange={e => setPassword(e.target.value)} onKeyPress={e => e.key === 'Enter' && onLogin()} />
            <button onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-cloud-gray-dark hover:text-cloud-gray-deeper transition-colors">
              {showPassword ? <EyeOffIcon size={20} /> : <EyeIcon size={20} />}
            </button>
          </div>

          {/* Login Button */}
          <Button variant="primary" className="w-full mb-4" onClick={onLogin}>
            Sign In
          </Button>
