...
import { FolderIcon, ImageIcon, SettingsIcon, Grid3x3Icon, MonitorIcon, CpuIcon, HardDriveIcon, ShoppingBagIcon, TrashIcon, FileTextIcon, CodeIcon } from 'lucide-react';
...
export function Dock({
  ...
  return <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-slide-up">
      <div className="bg-white/60 dark:bg-dark-bg-light/60 backdrop-blur-cloud rounded-cloud-xl shadow-cloud-lg border border-cloud-gray/30 dark:border-dark-border px-3 py-3 flex items-center gap-2">
        ...
        {apps.map(app => {
        const Icon = app.icon;
        return <button key={app.id} onClick={() => onAppClick(app.id)} className={`
                ...
                ${activeApp === app.id ? 'ring-2 ring-cloud-green ring-offset-2' : ''}
              `}>
              <Icon size={24} className="text-white" />
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-3 py-1 bg-dark-bg-light text-white text-xs rounded-cloud opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                {app.name}
              </div>
              {activeApp === app.id && <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-cloud-green rounded-full" />}
            </button>;
      })}
        ...
        return <button key={app.id} onClick={() => onAppClick(app.id)} className={`
                ...
                ${activeApp === app.id ? 'ring-2 ring-cloud-green ring-offset-2' : ''}
              `}>
              <Icon size={24} className="text-cloud-gray-deeper dark:text-dark-text" />
              <div className="absolute -top-8 left-1/2 -translate-x-1/2 px-3 py-1 bg-dark-bg-light text-white text-xs rounded-cloud opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                {app.name}
              </div>
              {activeApp === app.id && <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 bg-cloud-green rounded-full" />}
